...
def main():
    ...
    if args.algorithm_cfg is not None:
        with open(args.algorithm_cfg, 'r', encoding='utf-8') as f:
            new_cfg = yaml.full_load(f)
        parser.set_defaults(**new_cfg)
        args = parser.parse_args()
    ...
    with open(osp.join(args.out, 'config.yaml'), 'w', encoding='utf-8') as f:
    ...
    # writing log
    with open(osp.join(args.out, 'log.txt'), 'a', encoding='utf-8') as f:
        f.write(record_str)
